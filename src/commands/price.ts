...
export = {
  ...
  async execute(interaction, config: Config) {
    ...
    if (ticker) {
        fmp.stock(ticker).quote().then(response => {
            console.log(response);
            return interaction.reply(`Current price of ${ticker}: $${response[0].price} (${response[0].changesPercentage}%), volume: ${response[0].volume}`);
        }).catch((error) => {
            console.log(error);
